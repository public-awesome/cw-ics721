...
use ics721::{token_types::ClassId, NonFungibleTokenPacketData};
...
pub fn ibc_packet_ack(
    ...
    if let Some(error) = try_get_ack_error(&ack.acknowledgement) {
        ...
        let callback =
            match ack_callback_msg(deps.as_ref(), ics721::Ics721Status::Success, msg.clone()) {
                Some(msg) => vec![msg],
                None => vec![],
            };
...
fn handle_packet_fail(
    ...
    let callback =
        match ack_callback_msg(deps.as_ref(), ics721::Ics721Status::Failed, message.clone()) {
            Some(msg) => vec![msg],
            None => vec![],
        };
