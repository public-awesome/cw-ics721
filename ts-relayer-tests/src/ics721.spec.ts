...
import { adminCleanAndBurnNft, adminCleanAndUnescrowNft, incomingChannels, migrate, migrateIncomingProxy, nftContracts, outgoingChannels } from './ics721-utils';
import {
  assertAckErrors,
  assertAckSuccess,
  bigIntReplacer,
...
const standardSetup = async (t: ExecutionContext<TestContext>) => {
    ...
    cw721: {
      ...
      instantiateMsg: {
        ...
        collection_info_extension: {
          ...
          royalty_info: {
            payment_address: t.context.wasmClient.senderAddress,
            share: 0.1,
          },
        },
    ...
    cw721: {
      ...
      instantiateMsg: {
        ...
        collection_info_extension: {
          ...
          explicit_content: false,
          royalty_info: {
            payment_address: t.context.osmoClient.senderAddress,
            share: 0.1,
          },
        },
  ...
  const wasmCw721IncomingProxyId = info.wasmContractInfos.cw721IncomingProxy.codeId;
  t.context.wasmCw721IncomingProxyId = wasmCw721IncomingProxyId;
  const osmoCw721IncomingProxyId = info.osmoContractInfos.cw721IncomingProxy.codeId;

  const wasmIcs721Id = info.wasmContractInfos.ics721.codeId;
  const osmoIcs721Id = info.osmoContractInfos.ics721.codeId;

  const wasmCw721OutgoingProxyId = info.wasmContractInfos.cw721OutgoingProxy.codeId;
  const osmoCw721OutgoingProxyId = info.osmoContractInfos.cw721OutgoingProxy.codeId;
  ...
  const { contractAddress: wasmIcs721 } = await instantiateContract(wasmClient, wasmIcs721Id, { cw721_base_code_id: wasmCw721Id }, 'label ics721');
  ...
  const { contractAddress: osmoIcs721 } = await instantiateContract(osmoClient, osmoIcs721Id, { cw721_base_code_id: osmoCw721Id }, 'label ics721');
  t.log(`- osmo ICS721 contract address: ${osmoIcs721}`);
  t.context.osmoIcs721 = osmoIcs721;

  t.log(`creating IBC connection and channel between ${wasmIcs721} <-> ${osmoIcs721}`);
  const channelInfo = await createIbcConnectionAndChannel(wasmClient, osmoClient, wasmIcs721, osmoIcs721, Order.ORDER_UNORDERED, 'ics721-1');
  t.log(`- channel for incoming proxy on both chains: ${JSON.stringify(channelInfo.channel, bigIntReplacer, 2)}`);
  t.context.channel = channelInfo;

  t.log(`instantiating wasm cw721-incoming-proxy (${wasmCw721IncomingProxyId}) with channel ${channelInfo.channel.src.channelId}`);
  ...
  t.log(`migrate ${wasmIcs721} contract with incoming proxy ${wasmCw721IncomingProxy}`);
  await migrate(wasmClient, wasmIcs721, wasmIcs721Id, wasmCw721IncomingProxy);

  const onlyOsmoIncomingChannelInfo = await createIbcConnectionAndChannel(wasmClient, osmoClient, wasmIcs721, osmoIcs721, Order.ORDER_UNORDERED, 'ics721-1');
  t.log(`- channel for incoming proxy only on wasm chain: ${JSON.stringify(onlyOsmoIncomingChannelInfo.channel, bigIntReplacer, 2)}`);
  t.context.onlyOsmoIncomingChannel = onlyOsmoIncomingChannelInfo;

  t.log(`instantiating osmo cw721-incoming-proxy (${osmoCw721IncomingProxyId}) with channel ${channelInfo.channel.dest.channelId}and ${onlyOsmoIncomingChannelInfo.channel.dest.channelId}`);
  const { contractAddress: osmoCw721IncomingProxy } = await instantiateContract(
    osmoClient,
    osmoCw721IncomingProxyId,
    {
      origin: osmoIcs721,
      channels: [channelInfo.channel.dest.channelId, onlyOsmoIncomingChannelInfo.channel.dest.channelId],
    },
    'label incoming proxy'
  );
  ...
  t.log(`instantiating wasm cw721-outgoing-proxy (${wasmCw721OutgoingProxyId}) with ${per_block} per blocks rate limit`);
  ...
  t.log(`instantiating osmo cw721-outgoing-proxy (${osmoCw721OutgoingProxyId}) with ${per_block} per blocks rate limit`);
  ...
  t.log(`migrate ${wasmIcs721} contract with incoming (${wasmCw721IncomingProxy}) and outgoing proxy (${wasmCw721OutgoingProxy})`);
  await migrate(wasmClient, wasmIcs721, wasmIcs721Id, wasmCw721IncomingProxy, wasmCw721OutgoingProxy);

  t.log(`migrate ${osmoIcs721} contract with incoming (${osmoCw721IncomingProxy}) and outgoing proxy (${osmoCw721OutgoingProxy})`);
  await migrate(osmoClient, osmoIcs721, osmoIcs721Id, osmoCw721IncomingProxy, osmoCw721OutgoingProxy);

  t.log(`creating another IBC connection and channel between wasm and osmo (${wasmIcs721} <-> ${osmoIcs721})`);
  const otherChannelInfo = await createIbcConnectionAndChannel(wasmClient, osmoClient, wasmIcs721, osmoIcs721, Order.ORDER_UNORDERED, 'ics721-1');
  t.log(`- other channel not WLed for incoming proxy: ${JSON.stringify(otherChannelInfo.channel, bigIntReplacer, 2)}`);
...
test.serial('transfer NFT: wasmd -> osmo', async (t) => {
  ...
  let transferResponse = await sendNft(wasmClient, wasmCw721, wasmCw721OutgoingProxy, ibcMsg, tokenId);
  ...
  t.log(`transfering to osmo chain via unknown ${otherChannel.channel.src.channelId}`);
  const beforeWasmOutgoingClassTokenToChannelList = await outgoingChannels(wasmClient, wasmIcs721);
  const beforeWasmIncomingClassTokenToChannelList = await incomingChannels(wasmClient, wasmIcs721);
  const beforeWasmNftContractsToClassIdList = await nftContracts(wasmClient, wasmIcs721);
  const beforeOsmoOutgoingClassTokenToChannelList = await outgoingChannels(osmoClient, osmoIcs721);
  const beforeOsmoIncomingClassTokenToChannelList = await incomingChannels(osmoClient, osmoIcs721);
  const beforeOsmoNftContractsToClassIdList = await nftContracts(osmoClient, osmoIcs721);
  ...
  const afterWasmIncomingClassTokenToChannelList = await incomingChannels(wasmClient, wasmIcs721);
  const afterWasmNftContractsToClassIdList = await nftContracts(wasmClient, wasmIcs721);
  t.deepEqual(
...
- wasm after: ${JSON.stringify(afterWasmNftContractsToClassIdList)}`
  ...
  const afterOsmoOutgoingClassTokenToChannelList = await outgoingChannels(osmoClient, osmoIcs721);
  const afterOsmoIncomingClassTokenToChannelList = await incomingChannels(osmoClient, osmoIcs721);
  const afterOsmoNftContractsToClassIdList = await nftContracts(osmoClient, osmoIcs721);
  t.deepEqual(
...
- osmo after: ${JSON.stringify(afterOsmoNftContractsToClassIdList)}`
  ...
  // test transfer NFT to osmo chain
  t.log(`transfering to osmo chain via ${onlyOsmoIncomingChannel.channel.src.channelId}`);
  ibcMsg = {
    receiver: osmoAddr,
  ...
  let wasmOutgoingClassTokenToChannelList = await outgoingChannels(wasmClient, wasmIcs721);
  t.log(`- outgoing channels: ${JSON.stringify(wasmOutgoingClassTokenToChannelList)}`);
  t.true(wasmOutgoingClassTokenToChannelList.length === 1, `outgoing channels must have one entry: ${JSON.stringify(wasmOutgoingClassTokenToChannelList)}`);
  ...
  t.log(`transfering back to wasm chain via unknown ${onlyOsmoIncomingChannel.channel.dest.channelId}`);
  transferResponse = await sendNft(
    osmoClient,
    osmoCw721,
    t.context.osmoCw721OutgoingProxy,
  ...
  t.log(`migrate ${wasmCw721IncomingProxy} contract and add channel ${onlyOsmoIncomingChannel.channel.src.channelId}`);
  await migrateIncomingProxy(wasmClient, wasmCw721IncomingProxy, wasmCw721IncomingProxyId, [channel.channel.src.channelId, onlyOsmoIncomingChannel.channel.src.channelId]);

  // test back transfer NFT to wasm chain, where onlyOsmoIncomingChannel is not WLed on wasm chain
  t.log(`transfering back to wasm chain via WLed ${onlyOsmoIncomingChannel.channel.dest.channelId}`);
  transferResponse = await sendNft(
    osmoClient,
    osmoCw721,
    t.context.osmoCw721OutgoingProxy,
  ...
  t.true(nftContractsToClassIdList.length === 1, `nft contracts must have exactly one entry: ${JSON.stringify(nftContractsToClassIdList)}`);
  ...
  wasmOutgoingClassTokenToChannelList = await outgoingChannels(wasmClient, wasmIcs721);
  t.true(wasmOutgoingClassTokenToChannelList.length === 0, `outgoing channels not empty: ${JSON.stringify(wasmOutgoingClassTokenToChannelList)}`);
...
test.serial('admin unescrow and burn NFT: wasmd -> osmo', async (t) => {
  ...
  const { wasmClient, wasmAddr, wasmCw721, wasmIcs721, wasmCw721OutgoingProxy, osmoClient, osmoAddr, osmoIcs721, channel } = t.context;
  ...
  const transferResponse = await sendNft(wasmClient, wasmCw721, wasmCw721OutgoingProxy, ibcMsg, tokenId);
  t.truthy(transferResponse);
...
- before: ${JSON.stringify(beforeOsmoNftContractsToClassIdList)}`
  ...
  await adminCleanAndUnescrowNft(wasmClient, wasmIcs721, wasmAddr, tokenId, wasmCw721, wasmCw721);
...
- after: ${JSON.stringify(afterWasmOutgoingClassTokenToChannelList)}`
  ...
  await adminCleanAndBurnNft(osmoClient, osmoIcs721, osmoAddr, tokenId, osmoClassId, osmoCw721);
  t.log(`- response: ${JSON.stringify(response, bigIntReplacer, 2)}`);
  allNFTs = await allTokens(osmoClient, osmoCw721);
...
test.serial('malicious NFT', async (t) => {
  await standardSetup(t);
  const { wasmClient, wasmAddr, wasmIcs721, wasmCw721OutgoingProxy, osmoClient, osmoAddr, osmoIcs721, osmoCw721OutgoingProxy, channel } = t.context;
  const tokenId = '1';
  ...
  let transferResponse = await sendNft(wasmClient, cw721, wasmCw721OutgoingProxy, ibcMsg, tokenId);
  t.truthy(transferResponse);
  ...
  transferResponse = await sendNft(osmoClient, osmoCw721, osmoCw721OutgoingProxy, ibcMsg, tokenId);
  t.truthy(transferResponse);
  // before relay NFT escrowed by ICS721
  tokenOwner = await ownerOf(osmoClient, osmoCw721, tokenId);
  t.is(osmoIcs721, tokenOwner.owner);
